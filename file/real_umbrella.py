...
dag = DAG(
    dag_id="real_umbrella_bangkok",
    default_args=default_args,
    description="Fetch weather/sales data, clean, join, train ML model, deploy. Custom task for Bangkok weather comparison.",
    schedule_interval="@daily",
)
 
# 1. Получение прогноза погоды для Бангкока
def fetch_weather_forecast():
    api_key = "f1d1b8c466274f69b5e85848252103"  # замените на ваш API ключ
    url = f"http://api.weatherapi.com/v1/forecast.json?key={api_key}&q=Bangkok&days=7"  # Изменено на Бангкок
...
# 8. Сравнение температур первого и последнего дня
def compare_temperatures(**kwargs):
    data_dir = '/opt/airflow/data'
    df = pd.read_csv(os.path.join(data_dir, 'weather_forecast.csv'))
    ...
    if first_day_temp > last_day_temp:
        print("Первый день теплее.")
    elif first_day_temp < last_day_temp:
        print("Последний день теплее.")
    else:
        print("Температуры первого и последнего дня равны.")
...
# Новая задача для сравнения температур
compare_temperatures_task = PythonOperator(
    task_id="compare_temperatures",
    python_callable=compare_temperatures,
    dag=dag,
)
...
# Добавляем задачу сравнения температур после получения прогноза погоды
fetch_weather_task >> compare_temperatures_task